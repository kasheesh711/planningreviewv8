...
const NetworkGraphView = ({ rawData, bomData, isDarkMode, selectedNode }) => {
    ...
    useEffect(() => {
        ...
        const animate = () => {
            ...
            links.forEach(link => {
                ...
                if (isConnected) {
                    ctx.strokeStyle = isDarkMode ? '#38bdf8' : '#2563eb'; // Sky/Blue accent
                    ctx.globalAlpha = 0.8;
    ...
    return (
        <div className={`flex flex-col h-full rounded-2xl border shadow-sm overflow-hidden ${isDarkMode ? 'bg-slate-950 border-slate-800' : 'bg-white border-slate-200/70'}`}>
            {/* Controls Header */}
            <div className={`px-4 py-3 border-b flex items-center justify-between ${isDarkMode ? 'border-slate-800 bg-slate-900/80 backdrop-blur' : 'border-slate-100 bg-slate-50/80 backdrop-blur'}`}>
                 <div className="flex items-center gap-4">
                    <div className="flex items-center gap-2">
                        <Network className={`w-4 h-4 ${isDarkMode ? 'text-sky-400' : 'text-sky-600'}`} />
                        <h3 className={`font-semibold tracking-tight text-sm ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>Network Brain</h3>
                    </div>
                    ...
                    <select
                        ...
                        className={`text-[11px] border-none rounded bg-transparent font-semibold focus:ring-0 cursor-pointer ${isDarkMode ? 'text-slate-300' : 'text-slate-700'}`}
                    >
                 ...
                 {/* Zoom Controls */}
                 <div className="flex items-center gap-1 bg-slate-500/5 rounded-lg p-1">
                     <button onClick={() => simState.current.transform.k *= 1.2} className={`p-1.5 rounded-md transition-colors ${isDarkMode ? 'hover:bg-slate-800 text-slate-300' : 'hover:bg-slate-100 text-slate-600'}`}><Plus size={14}/></button>
                     <button onClick={() => simState.current.transform.k *= 0.8} className={`p-1.5 rounded-md transition-colors ${isDarkMode ? 'hover:bg-slate-800 text-slate-300' : 'hover:bg-slate-100 text-slate-600'}`}><Minus size={14}/></button>
                     <button onClick={() => {simState.current.transform = {x:0, y:0, k:0.75}; simState.current.alpha=0.5}} className={`p-1.5 rounded-md transition-colors ${isDarkMode ? 'hover:bg-slate-800 text-slate-300' : 'hover:bg-slate-100 text-slate-600'}`}><Maximize size={14}/></button>
                 </div>
            </div>

            {/* Canvas */}
            <div ref={containerRef} className={`flex-1 relative cursor-move overflow-hidden touch-none ${isDarkMode ? 'bg-slate-950' : 'bg-slate-50'}`}>
                <div className={`absolute inset-x-0 top-0 h-10 ${isDarkMode ? 'bg-gradient-to-b from-slate-900/60 to-transparent' : 'bg-gradient-to-b from-white/90 to-transparent'} pointer-events-none z-10`} />
                <canvas
                    ref={canvasRef}
                ...
                <div className="absolute bottom-3 left-3 pointer-events-none flex gap-3">
                    <div className="flex items-center gap-1.5 bg-black/30 backdrop-blur px-2.5 py-1.5 rounded-full text-[10px] text-white border border-white/10 shadow-sm">
                    ...
                    <div className="flex items-center gap-1.5 bg-black/30 backdrop-blur px-2.5 py-1.5 rounded-full text-[10px] text-white border border-white/10 shadow-sm">
                        <div className="w-2 h-2 rounded-full bg-cyan-400"></div>DC
                    </div>
                </div>
            </div>
        </div>
        ...
        <div className="relative group" ref={wrapperRef}>
            ...
            <button
                className={`w-full rounded-lg border px-3 py-2 text-xs flex items-center justify-between cursor-pointer transition-all duration-200 ease-in-out
                    ${isOpen
                        ? 'border-sky-500 ring-1 ring-sky-500/20 shadow-md shadow-sky-500/10'
                        : isDarkMode
...
const NodeCard = React.memo(({ node, onSelect, isActive, onOpenDetail, isDarkMode }) => {
    ...
    const activeClasses = isDarkMode
        ? "ring-1 ring-sky-500 bg-slate-900/40 border-sky-500/50 shadow-[0_0_15px_rgba(56,189,248,0.18)]"
        ...
        >
            <div className="flex justify-between items-start mb-1.5">
                <div className="flex items-center gap-2 flex-1 min-w-0">
                    <span className={`text-[9px] font-bold uppercase px-1.5 py-0.5 rounded border tracking-wide
                ...
                <button 
                    ...
                    className={`p-1.5 rounded-lg transition-colors z-10 relative ${isDarkMode ? 'hover:bg-slate-700 text-slate-400 hover:text-sky-400' : 'hover:bg-slate-100 text-slate-400 hover:text-sky-600'}`}
                    title="View Details"
                >
...
const RenderColumn = React.memo(({ title, count, items, type, searchTerm, setSearchTerm, setSort, sortValue, isActiveCol, isDarkMode, children }) => (
    <div className={`flex flex-col h-full min-h-0 border-r ${isDarkMode ? 'border-slate-800 bg-slate-900/20' : 'border-slate-200/60 bg-slate-50/40'} ${isActiveCol ? (isDarkMode ? 'bg-sky-900/10' : 'bg-sky-50/40') : ''} min-w-[300px] flex-1`}>
    ...
    return (
        <div className={`min-h-screen pb-20 transition-colors duration-300 ${isDarkMode ? 'bg-slate-950 text-slate-200' : 'bg-slate-50 text-slate-900'}`}>
             ...
             <header className={`sticky top-0 z-40 backdrop-blur-md border-b transition-colors duration-300 ${isDarkMode ? 'bg-slate-900/80 border-slate-800' : 'bg-white/80 border-slate-200/60'}`}>
                <div className="max-w-[1800px] mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
                    <div className="flex items-center space-x-3">
                        <div className="bg-gradient-to-br from-sky-500 to-blue-600 p-2 rounded-xl shadow-lg shadow-sky-500/20 ring-1 ring-sky-400/40">
                            <Package className="w-5 h-5 text-white" />
                        </div>
                        <div>
                            <h1 className={`text-lg font-semibold tracking-tight leading-tight ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>Supply Chain <span className="text-sky-500">Center</span></h1>
                            <p className="text-[10px] font-semibold text-slate-500 uppercase tracking-[0.24em]">Inventory Intelligence</p>
                        </div>
                    </div>
                    <div className="flex items-center space-x-3">
                        <button onClick={() => setIsDarkMode(!isDarkMode)} className={`p-2 rounded-xl border transition-all ${isDarkMode ? 'bg-slate-800 border-slate-700 text-amber-300 hover:text-amber-200' : 'bg-white border-slate-200 text-slate-500 hover:text-sky-600'}`}>
                        ...
                        <label className={`group flex items-center px-3 py-2 border rounded-xl cursor-pointer transition-all text-xs font-medium ${isDarkMode ? 'bg-slate-800 border-slate-700 hover:bg-slate-700 text-slate-300' : 'bg-white border-slate-200 hover:bg-slate-50 text-slate-700'}`}>
            ...
            <div className="flex min-h-[calc(100vh-64px)] max-w-[1800px] mx-auto">
                ...
                <div className={`hidden xl:flex flex-col w-[5%] 2xl:w-[4%] border-r py-6 items-center gap-4 ${isDarkMode ? 'border-slate-800 bg-slate-900/50' : 'border-slate-200 bg-slate-50/50'}`}>
                    <button
                        ...
                        className={`p-3 rounded-xl transition-all group relative ${activeView === 'dashboard'
                            ? (isDarkMode ? 'bg-sky-500/20 text-sky-300' : 'bg-sky-50 text-sky-700')
                    ...
                    {activeView === 'dashboard' ? (
                        ...
                        <>
                            ...
                            <div className={`h-[600px] shrink-0 rounded-2xl shadow-sm border p-0 overflow-hidden flex flex-col transition-all duration-300 ${isDarkMode ? 'bg-slate-900 border-slate-800 shadow-black/20' : 'bg-white border-slate-200/60 shadow-slate-200/50'}`}>
                                <div className={`p-4 border-b flex items-center justify-between ${isDarkMode ? 'border-slate-800 bg-slate-900' : 'border-slate-100 bg-slate-50/70'}`}>
                                    <div className="flex items-center space-x-3">
                                        <div className="p-2 rounded-lg bg-sky-500/10 text-sky-600 ring-1 ring-sky-500/15"><Network className="w-5 h-5" /></div>
                                    ...
                                    <div className="flex items-center gap-2">
                                        <button
                                            ...
                                            className={`flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium border rounded-lg shadow-sm transition-all ${isDarkMode ? 'bg-slate-800/90 border-slate-700 text-slate-200 hover:bg-slate-700' : 'bg-white/95 border-slate-300 text-slate-700 hover:border-sky-300 hover:text-sky-700'}`}
                                    ...
                                    {ganttData.length > 0 ? (
                                        ganttData.map((row, idx) => (
                                            ...
                                            ${isDarkMode ? 'border-slate-800 hover:bg-slate-800' : 'border-slate-50 hover:bg-slate-50'}
                                                ${selectedItem && selectedItem.itemCode === row.itemCode && selectedItem.invOrg === row.invOrg ? (isDarkMode ? 'bg-sky-900/20' : 'bg-sky-50/70') : ''}`}>
                                                <div className={`flex-shrink-0 px-6 py-2 border-r truncate cursor-pointer h-full flex flex-col justify-center ${isDarkMode ? 'border-slate-800' : 'border-slate-50'}`} style={{ width: Y_AXIS_WIDTH }} onClick={() => setSelectedItem({ itemCode: row.itemCode, invOrg: row.invOrg })}>
                                                    <div className={`font-bold text-sm truncate transition-colors ${isDarkMode ? 'text-slate-300 group-hover:text-sky-300' : 'text-slate-700 group-hover:text-sky-700'}`}>{row.itemCode}</div>
                                                ...
                                                <div className="flex-1 relative h-full cursor-pointer" style={{ marginLeft: '20px', marginRight: '30px' }} onClick={() => setSelectedItem({ itemCode: row.itemCode, invOrg: row.invOrg })}>
                                                    ...
                                                    {row.blocks.map((block, bIdx) => {
                                                        ...
                                                        const colorClass = isCritical ? 'bg-red-500 ring-1 ring-red-300/60 shadow-lg shadow-red-500/20' : 'bg-amber-500 ring-1 ring-amber-300/50 shadow-lg shadow-amber-500/15';
                                                        return (
                                                            <div key={bIdx} className={`absolute h-4 top-4 rounded-sm cursor-pointer hover:scale-y-110 transition-transform ${colorClass}`} style={{...style, minWidth: '8px'}} title={`${block.status}: ${block.days} Days`}></div>
                                                        );
                                                    })}
                                                </div>
                                            </div>
                ...
                {/* --- RIGHT SIDEBAR (20%) --- */}
                <aside className={`w-[300px] 2xl:w-[20%] border-l h-screen sticky top-0 overflow-y-auto z-30 shadow-xl flex flex-col transition-colors duration-300 ${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200/80'}`}>
                    ...
                    <div className="p-5 flex-1 flex flex-col gap-6">
                        <div className="flex items-center justify-between">
                            ...
                            <button onClick={resetFilters} className="text-[10px] text-sky-600 hover:text-sky-500 font-medium uppercase tracking-wider">Reset All</button>
                        </div>
                        ...
                        <div className="space-y-5">
                            <div>
                                ...
                                <button onClick={() => setIsLeadTimeMode(!isLeadTimeMode)} className={`flex items-center justify-between w-full px-3 py-2 rounded-lg border transition-all ${isLeadTimeMode ? 'bg-sky-500/10 border-sky-500/50 text-sky-600' : isDarkMode ? 'bg-slate-800 border-slate-700 text-slate-400' : 'bg-white border-slate-200 text-slate-700'}`}>
                    ...
                    <div className={`fixed inset-x-0 bottom-0 z-50 backdrop-blur-xl border-t shadow-[0_-10px_40px_-15px_rgba(0,0,0,0.2)] transform transition-all duration-300 ease-in-out h-96 flex flex-col animate-in slide-in-from-bottom-10 ${isDarkMode ? 'bg-slate-900/95 border-slate-800' : 'bg-white/95 border-slate-200'}`}>
                        <div className={`px-6 py-4 border-b flex items-center justify-between ${isDarkMode ? 'bg-slate-900/50 border-slate-800' : 'bg-white/50 border-slate-100'}`}>
                            <div className="flex items-center space-x-4">
                                <div className="bg-sky-500/10 p-2 rounded-lg text-sky-600 ring-1 ring-sky-500/15"><Table className="w-5 h-5" /></div>
                                ...
                                <thead className={`text-xs uppercase sticky top-0 z-10 font-semibold tracking-wider backdrop-blur-sm ${isDarkMode ? 'bg-slate-900/90 text-slate-400' : 'bg-slate-50/90 text-slate-500'}`}>
                                    <tr>
                                        ...
                                        {selectedItemData.dates.map(dateStr => (
                                            <th key={dateStr} className={`px-3 py-3 border-b text-center min-w-[80px] transition-colors cursor-default ${isDarkMode ? 'border-slate-800 hover:bg-slate-800' : 'border-slate-200 hover:bg-sky-50/60'}`}>
